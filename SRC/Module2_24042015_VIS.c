//***********************************************************************
//**             Module 2 VIS of Canopy Height Program                 **
//***********************************************************************
...
#define NUM_LABELS    1
	...
	char	f_name_all[25]="_all_XYZ.psc"; 
	char	f_name_all1[250];
	...
	int i;
	int allID, allNo, allNoAll,allAmplitude;
	double allWidth, allEasting, allNorthing, allElevation, allAngle;
	...
	printf("***********************************************************************\n");
	printf("**             Module 2 VIS of Canopy Height Program                 **\n");
	printf("***********************************************************************\n");
	...
	strcpy(f_name_all1,identifier);

	strcat(f_name_all1,f_name_all);
	...
	fprintf(out_sum,"***********************************************************************\n");
	fprintf(out_sum,"**             Module 2 VIS of Canopy Height Program                 **\n");
	fprintf(out_sum,"***********************************************************************\n");
...
/********************************************************************************************************************************************************/
	 ...
	 // if there is no transmitted waveform recorded specify ringing peak ratio.
	{
		...
		printf("\nPlease specify mean noise within the data \n\t(suggested value above: %.2f):  ",noise_sum_rec/count_noise_rec);
	...
	/*******************************************************************************************************************************************************/
				...
				if (x6<E_min||E_min==-999)
					E_min=x6;
				if (y6<N_min||N_min==-999)
					N_min=y6;
				if (h6<Z_min||Z_min==-999)
					Z_min=h6;

				if (x6>E_max||E_max==-999)
					E_max=x6;
				if (y6>N_max||N_max==-999)
					N_max=y6;
				if (h6>Z_max||Z_max==-999)
					Z_max=h6;
		/***********************************************************************************************************************************************/	
	...
	/******************************************************************************************************************************************************************/
		...
		
				{
					...
					/*fprintf(out_fp2,"DISREGARD waveform number %d,", pulseID);
					fprintf(out_fp2,"%d,%lf,%lf,%lf,", pulseID,start_pulse_easting,start_pulse_northing,start_pulse_elevation);
							for (train_counter=0;train_counter<=number_of_bins-1;train_counter++)
							{
								fprintf(out_fp2,"%.0lf,",amplitude[train_counter]);
							}
					fprintf(out_fp2," \n");*/
	...
	fprintf(out_xyz," \n");
	//fprintf(out_fp1,"Broken waveforms: %d\n",waveform_broken);
	fprintf(out_sum,"\n***********************************************************************\n");
	fprintf(out_sum,"\n* DATA INFORMATION *\n");
    ...
    {
		allID=readininteger(in_fp2); //pulseID
		allNo=readininteger(in_fp2); //pulse number
		allNoAll=readininteger(in_fp2);
		allAmplitude=readininteger(in_fp2); //amplitude
		allWidth=readindouble(in_fp2); //width
		allEasting=readindouble(in_fp2); //easting
		allNorthing=readindouble(in_fp2); //northing
		allElevation=readindouble(in_fp2); //elevation
		allAngle=readindouble(in_fp2);
		
		plcol1( ( allElevation - Z_min ) / ( Z_max - Z_min ) );
		plstring3( 1, &allEasting, &allNorthing,&allElevation,"#(729)" );
	}
	plcol0( 1 );
	plschr( 0.,0.7 );
	...
	plschr( 0.,1.0 );
	plcol0( 2 );
	plmtex( "t", 1.0, 0.5, 0.5, "All detected points: (alt: 50; az: 45)" );
	...
	for ( i = 1; i <= total_number_of_points; i++ )
    {
		...
		plcol1( ( allElevation - Z_min ) / ( Z_max - Z_min ) );
		plstring( 1, &allEasting, &allNorthing,"#(729)" );
	}
	...
	plmtex("t",1.5, 0.5, 0.5, "All detected points: Plan view ");

	save_plot( f_name_all1 );
