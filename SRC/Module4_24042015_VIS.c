//***********************************************************************
//**               Module 4 VIS of Canopy Height Program               **
//***********************************************************************
//** Computes Leaf Area Index and Canopy Height Profiles               **
//** in gridded datasets. Provides visualization.                       **
//**                                                                   **
...
#define NUM_LABELS    1
	...
	char	f_name_LAI[10]="_LAI.psc"; 
	char	f_name_CHP[10]="_CHP.psc"; 
	char	f_name_LAI1[200], f_name_CHP1[200];
	...
	double	*energy,*energy_sum,*height_bin, *height_bin_plot;
	...
	double	b, single_amplitude, single_width,amplitude_trans,chp_total_max;
	...
	printf("***********************************************************************\n");
	printf("**               Module 4 VIS of Canopy Height Program               **\n");
	printf("***********************************************************************\n");
	printf("** Computes Leaf Area Index and Canopy Height Profiles               **\n");
	printf("** in gridded datasets. Provides visualization.                      **\n");
	printf("**                                                                   **\n");
	...
	fprintf(out_sum,"***********************************************************************\n");
	fprintf(out_sum,"**               Module 4 VIS of Canopy Height Program               **\n");
	fprintf(out_sum,"***********************************************************************\n");
	fprintf(out_sum,"** Computes Leaf Area Index and Canopy Height Profiles               **\n");
	fprintf(out_sum,"** in gridded datasets.Provides visualization.                       **\n");
	fprintf(out_sum,"**                                                                   **\n");
	...
	strcpy(f_name_LAI1,filename);
	strcpy(f_name_CHP1,filename);

	strcat(f_name_LAI1,f_name_LAI);
	strcat(f_name_CHP1,f_name_CHP);
	...
	height_bin_plot=(double *)calloc(profile_bins+3*trans_width,sizeof(double));
	...
	printf("\n*************************************************************************\n");
	//fprintf(out_sum,"Processed %d %m %Y at %H : %M \n", tm_mday,tm_mon,tm_year,tm_hour,tm_min);
	fprintf(out_sum,"\nUSER INPUT VALUES\n");
	...
	{
		height_bin_plot[i]=height_bin[i]-(sample*vectorZ/2);
...
//END OF OPENING FILES AND CREATING OUTPUT FILES	
...

	{
		...
		for (i=1;i<=ygrid_total;i++)
		{
			printf("Reflectance ratio row %d: ", i);

			if (input_YN[0]=='Y'||input_YN[0]=='y')
				fprintf(out_fp5,"Reflectance ratio row %d:", i);
		...
		if ((ygrid_total*xgrid_total)-count_no_data!=0)
		{
			...
			printf("\nTotal site LAI (LAI1) %f,", Total_LAI);
		}
		else
		{
		...
		//(assigning the cells with no ground return a mean value of total LAI calculated for the extreem case (TOTAL_LAI) - with the maximum value)
		for (i=1;i<=ygrid_total;i++)
		{
			for (j=1;j<=xgrid_total;j++)
			{
		...
		printf("\nTotal site LAI (LAI2) %f,", Total_LAI_corrected);
		...
		printf("\nTotal site reflectance ratio %f\n", Total_refl);
		...
		for (j=1; j<=profile_bins+3*trans_width-1; j++)
		{
			if (chp_total_max<chp_total_norm[j])
				chp_total_max=chp_total_norm[j];
		}
		...
		plcol0( 1 );
		plenv( 0.0, (chp_total_max+chp_total_max*0.1), 0.0, profile_height+1, 0, 0 );
		...
		plmtex("r", 5.0, 0.5, 0.5, out_filename9);

		plline( (profile_bins+3*trans_width-1),chp_total_norm, height_bin_plot);

		printf("\nSave the CHP file: %s", f_name_CHP1);
		// save the plot
		save_plot( f_name_CHP1 );
		...
		//LAI_minimum=0;
		for ( i = 0; i < ns; i++ )
			clevel[i] = 0 + ( LAI_maximum - 0 ) * ( i + 0.5 ) / (double) ns;

		for ( i = 0; i < ns + 1; i++ )
			shedge[i] = 0 + ( LAI_maximum  - 0 ) * (double) i / (double) ns;
		...
		for (i=1;i<=ygrid_total;i++)
		{
			y_LAI[0]=y_ll+(i-1)*ycell_size;
			y_LAI[1]=y_ll+(i-1)*ycell_size;
			y_LAI[2]=y_ll+(i)*ycell_size;
			y_LAI[3]=y_ll+(i)*ycell_size;

			for (j=1;j<=xgrid_total;j++)
			{					
				x_LAI[0]=x_ll+(j-1)*xcell_size;
				x_LAI[1]=x_ll+(j)*xcell_size;
				x_LAI[2]=x_ll+(j)*xcell_size;
				x_LAI[3]=x_ll+(j-1)*xcell_size;

				//Assign colour to every cell according to LAI value
				plcol1(LAI_array[i][j]/ (LAI_maximum));	
		...
		// Name the axes
		plschr( 0.,1.0 );
		plmtex("b",3.0, 0.32, 0, "Easting [m]");
		plmtex("l",5.5, 0.3, 0, "Northing [m]");
		plmtex("t",2.0, 0.36, 0, "LAI MAP ");
		//pllab( "Easting [m]", "Northing [m]", "LAIe MAP " );

		plfont( 1 );
		plschr( 0., 0.7 );
		plmtex("r", 10.0, 0.5, 0.5, out_filename8);
		sprintf(string, "grid: %.1lfm by %.1lfm", xcell_size, ycell_size);
		plmtex("t", 5.5, 1.1, 1, string);
		sprintf(string, "area: %.1lfm by %.1lfm", xdim, ydim );
		plmtex("t", 4.0, 1.1, 1, string);
		sprintf(string, "LLC: %.0lfm, %.0lfm", x_ll, y_ll );
		plmtex("t", 2.5, 1.1, 1, string);
		sprintf(string, "Total LAIe: %.2f",Total_LAI );
		plmtex("t", 1.0, 1.1, 1, string);

		printf("\nSave the LAI file: %s", f_name_LAI1);
		// save the plot
		save_plot( f_name_LAI1 );
